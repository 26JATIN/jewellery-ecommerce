  ...
  return (
    <div className={`relative w-full h-full overflow-hidden ${className || ''}`}>
      ...
      {(!isLoaded && !hasError) && category.image && (
        <div className="absolute inset-0 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 shimmer" />
      )}
...
export default function CategoryShowcase() {
  ...
  const filteredSubcategories = useMemo(() => {
    ...
    return subcategories.filter(sub => {
      ...
      // Safe comparison
      const match = (sub.category?._id && category._id && sub.category._id === category._id) ||
  ...
  const loadingSkeleton = useMemo(() => (
    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8 lg:gap-10">
      {[...Array(12)].map((_, index) => (
        <div key={index}>
          <div className="bg-gray-200 dark:bg-gray-800 rounded-3xl aspect-[4/3] mb-4 shimmer"></div>
          <div className="space-y-2">
            <div className="h-6 bg-gray-200 dark:bg-gray-800 rounded w-3/4 shimmer"></div>
            <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded w-1/2 shimmer"></div>
          </div>
        </div>
      ))}
    </div>

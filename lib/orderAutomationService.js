    ...
    async processNewOrder(orderId) {
        try {
            ...
            if (this.autoShipEnabled) {
                // Cancel any existing scheduled task for this order
                this.cancelScheduledTask(orderId);
                ...
                const timeoutId = setTimeout(async () => {
                    try {
                        ...
                        // Clean up the task from map after execution
                        this.scheduledTasks.delete(orderId);
                    }
                }, this.autoShipDelay * 60 * 1000);
                
                // Store the timeout ID for potential cleanup
                this.scheduledTasks.set(orderId, timeoutId);
                console.log(`Scheduled auto-ship for order ${orderId} in ${this.autoShipDelay} minutes`);
            }

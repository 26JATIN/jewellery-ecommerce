...
class ReversePickupService {
    ...
    async createReversePickup(returnId) {
        try {
            ...
            // Create reverse pickup order in Shiprocket 
            const response = await shiprocket.createReversePickup(returnData);
    ...
    async automateReversePickup(returnId) {
        try {
            ...
            const processResponse = await this.processReversePickup(returnId);
            
            console.log(`Automated reverse pickup completed for return: ${returnId}`);
            return {

...
class App {
  ...
  onWheel(e) {
    ...
    // If horizontal scroll is detected, use it
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      e.preventDefault();
      this.scroll.target += (deltaX > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2;
      this.onCheckDebounce();
    }
    // If it's mostly vertical scroll, let it pass through for page scrolling
  }
  onCheck() {
  ...
  addEventListeners() {
    ...
    // Container-specific events to avoid interfering with page scroll
